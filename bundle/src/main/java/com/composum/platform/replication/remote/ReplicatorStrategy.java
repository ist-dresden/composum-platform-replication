...
public class ReplicatorStrategy {
    ...
    public void replicate() throws ReleaseChangeEventListener.ReplicationFailedException {
        ...
        try {
            ...
            RemotePublisherService.LOG.info("Changed paths below {}: {}", commonParent, changedPaths);
            ...
            for (String path : pathsToTransmit) {
                ...
                if (status == null || !status.isValid()) {
                    ...
                    throw new ReleaseChangeEventListener.ReplicationFailedException("Remote upload failed for " + replicationConfig + " " +
                            "path " + path, null, null);
                } else {
            ...
            if (!status.isValid()) {
                ...
                progress = 0;
                throw new ReleaseChangeEventListener.ReplicationFailedException("Remote commit failed for " + replicationConfig, null, null);
            }
            progress = 100;
            cleanupUpdateInfo = null;
            ...
            messages.add(Message.error("Remote publishing failed for {} because of {}",
                    ...
                    String.valueOf(e)), e);
            throw new ReleaseChangeEventListener.ReplicationFailedException("Remote publishing failed for " + replicationConfig, e, null);
        } finally {
    ...
    public ReleaseChangeEventPublisher.CompareResult compareTree(int details) throws RemotePublicationReceiverFacade.PublicationReceiverFacadeException, ReleaseChangeEventListener.ReplicationFailedException {
        try {
            ...
            RemotePublisherService.LOG.error("" + e, e);
